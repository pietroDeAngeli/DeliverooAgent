...
\caption{Multi-agent performance: Pure BDI vs LLM-powered agent scores.}
\label{tab:multi_agent_benchmark}
\begin{tabular}{lrrc}
\hline
\textbf{Map} & \textbf{Pure BDI} & \textbf{LLM} & \textbf{Tested} \\ \hline
...
The score differences across maps are primarily driven by the complexity of the natural language instructions provided to the agents: simpler prompts allow the BDI planner to operate with minimal LLM overhead, while more complex directives introduce latency and occasional misclassifications that reduce the effective score. 

It is worth noting that the reported scores could be higher if the server infrastructure provided a dedicated mechanism for awarding points based on textual prompt interactions; however, such a system is not strictly required, as bonus points of this kind are left to the discretion of the administrator.
